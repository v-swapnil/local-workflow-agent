  ...
  return (
    <div className="app-drag relative z-30 flex h-11 items-center justify-between border-b border-ink-800/60 bg-ink-950/90 pl-4 pr-4 backdrop-blur-md">
      <div className="flex items-center gap-2.5 font-mono text-ui-xs tracking-widest2 text-ink-500">
        <span className="uppercase">Local workflow agent</span>
        {version && (
          <Badge
            variant="outline"
            className="border-ink-700/60 bg-ink-800/60 font-mono text-ui-2xs text-ink-500"
          >
            {version}
          </Badge>
        )}
      ...
      <div className="app-no-drag flex items-center gap-2">
        ...
        <TooltipProvider delayDuration={400}>
          <Tooltip>
            ...
            <TooltipContent side="bottom" className="font-mono text-ui-2xs">
              Toggle theme (⌘⇧L)
            </TooltipContent>
          </Tooltip>
        </TooltipProvider>
        ...
        <StatusDot label={providerHealth.data?.provider ?? 'llm'} ok={providerHealth.data?.ok} />
...
function StatusDot({ label, ok, detail }: { label: string; ok?: boolean; detail?: string }) {
  ...
  return (
    <div className="flex items-center gap-1.5 rounded-md px-1.5 py-1 font-mono text-ui-2xs uppercase tracking-widest2 text-ink-500">
      <span
        className={`h-[5px] w-[5px] rounded-full ${color} ${ok === false ? 'animate-pulse' : ''}`}
      />
      <span>{label}</span>
